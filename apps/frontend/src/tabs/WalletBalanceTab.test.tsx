...
import { mockPaperLedger, mockPaperWallet, mockSimulatedOrders, mockWalletBalances } from "../api/mockData";
...
describe("WalletBalanceTab", () => {
  it("renders paper-only wallet, ledger, and simulated orders without forbidden copy", () => {
    ...
    expect(copy).not.toMatch(/fake-live|broker-ready|execution-ready|real-money/i);
  });

  it("renders honest unavailable copy when no paper wallet has loaded", () => {
    render(<WalletBalanceTab rows={[]} paperWallet={null} paperLedger={[]} simulatedOrders={[]} />);
